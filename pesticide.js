...
// This content script initializes the Pesticide extension functionality

// Listen for messages from the background script
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'togglePesticide') {
    togglePesticide(message.isActive);
    sendResponse({ success: true });
  }
});

// Function to toggle Pesticide CSS
function togglePesticide(isActive) {
  const pesticideStyleId = 'pesticide-for-chrome-style';

  // Check if pesticide is already active
  const existingStyle = document.getElementById(pesticideStyleId);

  if (isActive && !existingStyle) {
    // Create and inject the Pesticide CSS
    const style = document.createElement('style');
    style.id = pesticideStyleId;
    style.textContent = `
